...
当前项目和 Skill 标识固定为 `lsnu-campus-skill`。1.0／1.1／1.2.0 的历史压缩包保留原名与原始校验值；旧标识客户端应重新导入 1.2.1 或更新的完整包，不能把新标识冒充旧版本。个人 Skill 的独立名称与包外记忆目录继续复用，迁移不授权读取或上传记忆。

公共数据分支的 `skill_id` 同步更正并重新计算完整性哈希；只修正标识时不修改原采集时间或内容版本。更名后由 Actions 下一次真实采集推进时间。默认公共缓存目录为 `~/.cache/lsnu-campus-skill`；旧缓存不冒充新标识，允许重新下载。
